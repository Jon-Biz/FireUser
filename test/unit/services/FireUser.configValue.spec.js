...
describe("the FireUser service", function() {
  ...
  describe("when instantiated after a FireUserConfig value service is set", function() {
    ...
    beforeEach(function() {
      ...
      var FBopts = this.FBopts = {'datalocation':'datalocation'};      
    ...
    it("should extend FBopts with FireUserConfig to create the options", inject(function($fireUser) {
      expect($fireUser.options.url).toEqual(this.FBconfig.url)
      expect($fireUser.options.datalocation).toEqual(this.FBopts.datalocation);
    }));
